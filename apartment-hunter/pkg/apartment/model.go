...
func LoadApartment(id string, events []events.Event) *Apartment {
	m := &Apartment{
		id: id,
	}
...
func (m *Apartment) ID() string {
	return m.id
}

func (m *Apartment) Version() uint32 {
	return m.version
}

func (m *Apartment) GetChanges() []events.Event {
	return m.changes
}

func (m *Apartment) apply(event events.Event, save bool) {
	switch e := event.(type) {
	case events.Created:
		m.address = e.Address
		m.url = e.Url
...
func Create(id, address, url, title string, price float32,
	...
	body string, lng, lat float32, closed bool) *Apartment {
	m := &Apartment{
		id: id,
	}
	e := events.Created{
...
func (m *Apartment) TitleUpdate(title string) {
	if m.title == title {
		return
	}
...
func (m *Apartment) PriceUpdate(price float32) {
	if m.price == price {
		return
	}
...
func (m *Apartment) BedroomsUpdate(bedrooms uint64) {
...
func (m *Apartment) BathroomsUpdate(bathrooms float32) {
...
func (m *Apartment) SqftUpdate(sqft float32) {
	if m.sqft == sqft {
		return
	}
...
func (m *Apartment) AvailabilityDateUpdate(date string) {
...
func (m *Apartment) CatsUpdate(allowed bool) {
	if m.cats == allowed {
		return
	}
...
func (m *Apartment) DogsUpdate(allowed bool) {
	if m.dogs == allowed {
		return
	}
...
func (m *Apartment) HousingUpdate(housingType string) {
...
func (m *Apartment) WDUpdate(wdType string) {
	if m.wdType == wdType {
		return
	}
	e := events.WDUpdated{
...
func (m *Apartment) ParkingUpdate(parkingType string) {
...
func (m *Apartment) ImagesUpdate(images []string) {
...
func (m *Apartment) BodyUpdate(body string) {
	if m.body == body {
		return
	}
...
func (m *Apartment) LocationUpdate(lng, lat float32) {
